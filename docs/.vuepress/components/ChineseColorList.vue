...
const list = dict2List(chinese, (key, value) => [key, value.toUpperCase()])
...
<template>
  <div class="colorname-list">
    ...
    <ColorNameItem v-for="item in list" :name="item.key" :key="item.key"  :color="item.value" :widths="widths" />
  </div>
</template>
