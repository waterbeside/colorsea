...
2: (default value) only show alpha when alpha is not equal to 100% @return Returns an RGB hex string`}]},{path:"/pages/CreateInstance.html",title:"Create a Color instance object",pathLocale:"/",contents:[{header:"Create a Color instance object",slug:"create-a-color-instance-object",content:""},{header:"colorsea()",slug:"colorsea",content:"Create a color instance object using the colorsea(rgb, alpha, config) function."},{header:"Parameters",slug:"parameters",content:`colorInput: string | [number, number, number] Color data input (Required)
When the type is string, a string can be passed in several forms 1 hex string such as '#ffffff'，
2 Color space function string：For example'rgb(255, 255, 255)'、 'hsl(0, 20%, 50%)' etc.，This mode is supported only in v1.2.0 and later versions. For details, see the following description【简单示例-示例二】
3 Color name: After the color name is loaded, you can directly use the color name as a parameter. For details, see【 Use color names】 When the type is a [number, number, number] tuple, it is a rgb value: [r, b, b] alpha: number | undefined alpha channel (opacity), accepts only values in range [0, 100], (Optional)
When the parameter is undefined, if the colorInput parameter with an alpha value set, then the alpha value is set by colorInput, otherwise the default value is 100 config: Config Settings (Optional)
Note that this parameter is included after v1.2.0 type Config = { throwParseError: boolean // Whether to throw an error if parsing the color incorrectly. The default value is false. When false, enter the wrong color value and the default output is black.
}`},{header:"Simple Example",slug:"simple-example",content:`Example 1: Basic usage // You can pass in a HEX string
colorsea('#cc0020', 90)
// or [R, G, B]
...
// or
colorsea('rgba(204, 0, 32, 90%)') colorsea('#cc0020', 90)Example 2: Uses the color space function string
...
colorsea('hsl(150, 80%, 60%)')colorsea('hsla(150, 80%, 60%, 20%)')
...
colorsea('hsla(150, 80%, 60%, 20%)') - colorsea('hsla(150, 80%, 60%, 20%)', 40)colorsea('cmyk(40, 80, 60, 20)')
colorsea('cmyk(40, 80, 60, 20)')colorsea('hwb(180, 50%, 10%)')
// The other color space methods are similar and are not all shown here
colorsea('hwb(180, 50%, 10%)')TIP
If it contains parameters in the form of percentage, they can be expressed in the following types: Use percentage signs directly, such as:colorsea('rgba(204, 0, 32, 90%)')
...
The above three input colors are equal. What should be noted is:
...
This conversion only works if the colorInput argument is entered as a string. Unless otherwise noted, most percentages of colorsea are represented by numbers in the [0, 100] range from 0% to 100%`},{header:"Use color names",slug:"use-color-names",content:"The first parameter of colorsea() also supports passing in color names. When using these color names, you need to load the name mapping relationship table separately. The currently supported color names are X11 color names, Chinese traditional colors, Japanese Traditional Colors. You can also customize the color name. For specific usage, please refer to the document Using Color Names"},{header:"Other",slug:"other",content:`In addition to creating Color instance objects through the colorsea function, you can also create Color instances through other color space methods
...
When customizing color names, the keys of all color names must be in lowercase. Case insensitive when using`}]},{path:"/pages/QuickStart.html",title:"Quick Start",pathLocale:"/",contents:[{header:"Quick Start",slug:"quick-start",content:""},{header:"Installation",slug:"installation",content:"pnpm add colorsea yarn add colorsea npm install colorsea"},{header:"Import & Use",slug:"import-use",content:""},{header:"Import",slug:"import",content:`import colorsea from 'colorsea' const colorsea = require('colorsea') <script src="path/to/colorsea.js"><\/script>`},{header:"Color space conversion",slug:"color-space-conversion",content:`// ----- color conversion
...
1：展示alpha值，
2：(默认值)当alpha不等于100%才展示alpha @return 返回RGB十六进制字符串`}]},{path:"/zh/CreateInstance.html",title:"创建Color实例",pathLocale:"/zh/",contents:[{header:"创建Color实例",slug:"创建color实例",content:""},{header:"colorsea()",slug:"colorsea",content:"使用colorsea(colorInput, alpha, config)函数创建一个颜色实例对象，"},{header:"参数说明",slug:"参数说明",content:`colorInput: string | [number, number, number] 颜色数据输入（必填）
当类型为string时，可以传入以几种形式的字符串 1 hex字符串，如 '#ffffff'，
2 颜色空间函数字符串：如'rgb(255, 255, 255)'、 'hsl(0, 20%, 50%)' 等，此方式仅在v1.2.0及之后的版本支持，具体细节请参考后续的描述【简单示例-示例二】
3 颜色名称，加载颜色名称后，可直接使用该颜色的名称作为参数，具体用法请参考【颜色名称】 当类型为 [number, number, number]元组时，为rgb值 alpha: number | undefined alpha通道（不透明度），只接受范围为[0, 100]的数值, (非必填)
当此参数为undefined时，如果colorInput参数带有alpha值设置，则alpha值按colorInput设定，否则默认值为100 config: Config 设置 (非必填)
注意，此参数在v1.2.0之后才包含 type Config = { throwParseError: boolean // 如果解析颜色出错，是否抛出错误, 默认值为false。当为false时，输入错误的颜色值，默认会输出黑色。
}`},{header:"简单示例",slug:"简单示例",content:`示例1：基本用法 // 可以传入HEX字
colorsea('#cc0020', 90)
// or [R, G, B]
colorsea([204, 0, 32], 90)
// 也等同于
colorsea('rgb(204, 0, 32)', 90)
// 或
colorsea('rgba(204, 0, 32, 90%)') colorsea('#cc0020', 90)示例2：使用颜色空间函数字符串
...
// 各种颜色空间方法类似，此处不全部演示
colorsea('hwb(180, 50%, 10%)')TIP
其中如果包含有百分数形式的参数，可以以下几种型式表示： 直接使用百分号, 如：colorsea('rgba(204, 0, 32, 90%)')
可以不写百分号，输入90即为90%：colorsea('rgba(204, 0, 32, 90)')
使用小于1的小数，输入0.9即为90%: colorsea('rgba(204, 0, 32, 0.9)')
以上三种输入的颜色是相等的。 需要注意的是：
如果想输入0.1%则不能直接输入0.1，因为输入少于1的数，会自动乘以100，输入0.1即为10%。
所以，你可以直接输入0.1%, 或者0.001以代表0.1%
这种转换只有在字符串形式输入colorInput参数以有效，除非特别说明，colorsea的大部分百分数，都以[0, 100]范围的数字代表0%到100%`},{header:"使用颜色名称",slug:"使用颜色名称",content:"colorsea()的第一个参数还支持传入颜色名称，使用这些颜色名称时，要另外加载名称映射关系表，现支持的颜色名称有X11 color names、中国传统色、日本传统色。也可以自定义颜色名，具体用法请参看文档【使用颜色名称】"},{header:"其它",slug:"其它",content:`除了通过colorsea函数创建Color实例对象外，还可以通过其它颜色空间方法创建Color实例
const { rgb, hsl, hsv, hsi, hwb, xyz, lab, lch } = colorsea
/**
* 以上所有颜色空间方法，都接受4个参数，
...
自定义颜色名时，其所有颜色名称的key都必需为小写。使用时，不区分大小写`}]},{path:"/zh/QuickStart.html",title:"快速上手",pathLocale:"/zh/",contents:[{header:"快速上手",slug:"快速上手",content:""},{header:"安装",slug:"安装",content:"pnpm add colorsea yarn add colorsea npm install colorsea"},{header:"导入和使用",slug:"导入和使用",content:""},{header:"导入",slug:"导入",content:`import colorsea from 'colorsea' const colorsea = require('colorsea') <script src="path/to/colorsea.js"><\/script>`},{header:"颜色空间转换",slug:"颜色空间转换",content:`// ----- color conversion
colorsea('#ff0000').rgb() // [255, 0, 0]
colorsea('#ff0000', 50).rgba() // [255, 0, 0, 50]
...
#ff0000, #ee6666, return false, #ffff00, #000000, return true
TIP
color.red(),color.green(),color.blue(),color.hue(),color.saturation(),color.lightness(), color.alpha(),color.luma()等方法的取值，如果其计算结果为小数，将会返会所有小数位数，并不会将结果进行取整或截取多少位小数。`}]},{path:"/404.html",title:"",pathLocale:"/",contents:[]}],X="update-vuepress-plugin-full-text-search2-search-index";var k=g(q),D=E(()=>{const e=new Map;for(const o of k.value)e.set(o.path,o);return e});import.meta.webpackHot&&(__VUE_HMR_RUNTIME__[X]=e=>{k.value=e});function O(e){const o=g([]);let t=null;return P(e,()=>{t&&clearTimeout(t),t=setTimeout(a,100)}),o;function a(){const i=e.value.toLowerCase().trim();if(!i){o.value=[];return}const l=new Map,r=new Set;for(const n of k.value)for(const s of U(n,i)){r.add(s.parentPageTitle);let c=l.get(s.parentPageTitle);c||(c=[],l.set(s.parentPageTitle,c)),c.push(s)}const u=[...r].sort((n,s)=>{const c=l.get(n);return l.get(s).length-c.length});o.value=[...l].flatMap(([,n])=>n).sort((n,s)=>n.parentPagePriority-s.parentPagePriority||u.indexOf(n.parentPageTitle)-u.indexOf(s.parentPageTitle)||n.priority-s.priority)}}function*U(e,o){const t=x(e.title,o);if(t){yield{path:e.path,parentPageTitle:w(e),title:e.title,display:t,page:e,content:null,parentPagePriority:1,priority:1};return}for(const a of e.contents){const i=x(a.header,o);if(i){yield{path:e.path+(a.slug?`#${a.slug}`:""),parentPageTitle:w(e),title:e.title,display:i,page:e,content:null,parentPagePriority:10,priority:2};continue}const l=x(a.content,o);l&&(yield{path:e.path+(a.slug?`#${a.slug}`:""),parentPageTitle:w(e),title:e.title,display:[{type:"header",str:`${a.header}
`},...l],page:e,content:null,parentPagePriority:10,priority:10})}}function w(e){const o=e.path.split("/");let t="/";return o[1]&&(t=`/${o[1]}/`),(D.value.get(t)||e).title}function x(e,o){const t=[];let a=0;const i=e.toLowerCase().replace(/\s/gu," ");let l=0,r=i.indexOf(o,l);if(r<0)return null;for(;r>=0;){const n=r+o.length;if(u(e.slice(l,r),"normal"),u(e.slice(r,n),"highlight"),l=n,r=i.indexOf(o,l),a>100)break}return u(e.slice(l),"normal"),t.filter(n=>n.str);function u(n,s){let c=n;s==="normal"&&c.length>100&&a===0&&(c=`… ${c.slice(-10)}`);let f=!1;if(a+c.length>100){if(t.some(d=>d.type==="ellipsis"))return;c=c.slice(0,Math.max(100-a,1)),f=!0}t.push({type:s,str:c}),a+=c.length,f&&(t.push({type:"ellipsis",str:" …"}),a+=2)}}const G={},Y=M({name:"SearchBox",props:{locales:{type:Object,required:!1,default:()=>G}},setup(e){const{locales:o}=A(e),t=g(""),a=g(!1),i=g(-1),l=O(t),r=E(()=>t.value&&a.value&&l.value.length),u=N(),n=F(),s=E(()=>o.value[n.value]??{});function c(){if(!r.value)return;let m=i.value-1;m<0&&(m=l.value.length-1),d(m)}function f(){if(!r.value)return;let m=i.value+1;m>=l.value.length&&(m=0),d(m)}function d(m){i.value=m}function L(){i.value=-1}function R(m){if(!r.value)return;const I=l.value[m];I&&u.push(I.path)}return{query:t,focused:a,focusIndex:i,suggestions:l,activeSuggestion:r,onUp:c,onDown:f,focus:d,unfocus:L,go:R,locale:s}}});const $={class:"search-box",role:"search"},_=["placeholder"],V=["onMousedown","onMouseenter"],J=["href"],Q={key:0,class:"parent-page-title"},K={class:"suggestion-row"},Z={class:"page-title"},ee={class:"suggestion-content"};function oe(e,o,t,a,i,l){return h(),p("div",$,[H(b("input",{ref:"input","onUpdate:modelValue":o[0]||(o[0]=r=>e.query=r),"aria-label":"Search",class:C({focused:e.focused}),placeholder:e.locale.placeholder??"Search",autocomplete:"off",spellcheck:"false",onFocus:o[1]||(o[1]=()=>e.focused=!0),onBlur:o[2]||(o[2]=()=>e.focused=!1),onKeyup:[o[3]||(o[3]=v(r=>e.go(e.focusIndex),["enter"])),o[4]||(o[4]=v((...r)=>e.onUp&&e.onUp(...r),["up"])),o[5]||(o[5]=v((...r)=>e.onDown&&e.onDown(...r),["down"]))]},null,42,_),[[W,e.query]]),e.activeSuggestion?(h(),p("ul",{key:0,class:"suggestions",onMouseleave:o[7]||(o[7]=(...r)=>e.unfocus&&e.unfocus(...r))},[(h(!0),p(T,null,S(e.suggestions,(r,u)=>(h(),p("li",{key:u,class:C(["suggestion",{focused:u===e.focusIndex}]),onMousedown:n=>e.go(u),onMouseenter:n=>e.focus(u)},[b("a",{href:r.path,onClick:o[6]||(o[6]=j(()=>{},["prevent"]))},[r.parentPageTitle&&(!e.suggestions[u-1]||e.suggestions[u-1].parentPageTitle!==r.parentPageTitle)?(h(),p("div",Q,y(r.parentPageTitle),1)):z("v-if",!0),b("div",K,[b("div",Z,y(r.title||r.path),1),b("div",ee,[(h(!0),p(T,null,S(r.display,(n,s)=>(h(),p("span",{key:s,class:C(n.type)},y(n.str),3))),128))])])],8,J)],42,V))),128))],32)):z("v-if",!0)])}const ae=B(Y,[["render",oe],["__scopeId","data-v-c13c90dc"],["__file","SearchBox.vue"]]);export{ae as default};
