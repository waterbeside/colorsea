...
/** * Increase lightness * @param amount Brightness increase percentage, the default is 5, which means 5% * @param method If you fill in 'relative', it means that the parameter amount is a relative value * @returns Color */
...
/** * mix color * @param color Another color, which can be a Color instance, a hexadecimal color string, or an [r, g, b] color tuple * @param weight The mixing ratio of another color, the default value is 50 or 50% * @returns Color */
...
colorsea('山梗紫') // color: #61649F
colorsea('水がき') // color: #B9887D`}]},{path:"/pages/Value.html",title:"Color value and change value",pathLocale:"/",contents:[{header:"Color value and change value",slug:"color-value-and-change-value",content:""},{header:"color.red()",slug:"color-red",content:`color.red() => number
...
colorsea('山梗紫') // color: #61649F
colorsea('水がき') // color: #B9887D`}]},{path:"/zh/",title:"首页",pathLocale:"/zh/",contents:[]},{path:"/zh/Value.html",title:"颜色取值和改值",pathLocale:"/zh/",contents:[{header:"颜色取值和改值",slug:"颜色取值和改值",content:""},{header:"color.red()",slug:"color-red",content:`color.red() => number
...
colorsea('#22994a').luma() // 0.23616959154733871
TIP
color.red(),color.green(),color.blue(),color.hue(),color.saturation(),color.lightness(), color.alpha(),color.luma()等方法的取值，如果其计算结果为小数，将会返会所有小数位数，并不会将结果进行取整或截取多少位小数。`}]},{path:"/404.html",title:"",pathLocale:"/",contents:[]}],q="update-vuepress-plugin-full-text-search2-search-index";var k=g(W),D=w(()=>{const e=new Map;for(const o of k.value)e.set(o.path,o);return e});import.meta.webpackHot&&(__VUE_HMR_RUNTIME__[q]=e=>{k.value=e});function O(e){const o=g([]);let t=null;return N(e,()=>{t&&clearTimeout(t),t=setTimeout(a,100)}),o;function a(){const u=e.value.toLowerCase().trim();if(!u){o.value=[];return}const l=new Map,r=new Set;for(const n of k.value)for(const c of U(n,u)){r.add(c.parentPageTitle);let s=l.get(c.parentPageTitle);s||(s=[],l.set(c.parentPageTitle,s)),s.push(c)}const i=[...r].sort((n,c)=>{const s=l.get(n);return l.get(c).length-s.length});o.value=[...l].flatMap(([,n])=>n).sort((n,c)=>n.parentPagePriority-c.parentPagePriority||i.indexOf(n.parentPageTitle)-i.indexOf(c.parentPageTitle)||n.priority-c.priority)}}function*U(e,o){const t=E(e.title,o);if(t){yield{path:e.path,parentPageTitle:x(e),title:e.title,display:t,page:e,content:null,parentPagePriority:1,priority:1};return}for(const a of e.contents){const u=E(a.header,o);if(u){yield{path:e.path+(a.slug?`#${a.slug}`:""),parentPageTitle:x(e),title:e.title,display:u,page:e,content:null,parentPagePriority:10,priority:2};continue}const l=E(a.content,o);l&&(yield{path:e.path+(a.slug?`#${a.slug}`:""),parentPageTitle:x(e),title:e.title,display:[{type:"header",str:`${a.header}
`},...l],page:e,content:null,parentPagePriority:10,priority:10})}}function x(e){const o=e.path.split("/");let t="/";return o[1]&&(t=`/${o[1]}/`),(D.value.get(t)||e).title}function E(e,o){const t=[];let a=0;const u=e.toLowerCase().replace(/\s/gu," ");let l=0,r=u.indexOf(o,l);if(r<0)return null;for(;r>=0;){const n=r+o.length;if(i(e.slice(l,r),"normal"),i(e.slice(r,n),"highlight"),l=n,r=u.indexOf(o,l),a>100)break}return i(e.slice(l),"normal"),t.filter(n=>n.str);function i(n,c){let s=n;c==="normal"&&s.length>100&&a===0&&(s=`… ${s.slice(-10)}`);let f=!1;if(a+s.length>100){if(t.some(d=>d.type==="ellipsis"))return;s=s.slice(0,Math.max(100-a,1)),f=!0}t.push({type:c,str:s}),a+=s.length,f&&(t.push({type:"ellipsis",str:" …"}),a+=2)}}const G={},Y=P({name:"SearchBox",props:{locales:{type:Object,required:!1,default:()=>G}},setup(e){const{locales:o}=A(e),t=g(""),a=g(!1),u=g(-1),l=O(t),r=w(()=>t.value&&a.value&&l.value.length),i=F(),n=M(),c=w(()=>o.value[n.value]??{});function s(){if(!r.value)return;let m=u.value-1;m<0&&(m=l.value.length-1),d(m)}function f(){if(!r.value)return;let m=u.value+1;m>=l.value.length&&(m=0),d(m)}function d(m){u.value=m}function L(){u.value=-1}function R(m){if(!r.value)return;const I=l.value[m];I&&i.push(I.path)}return{query:t,focused:a,focusIndex:u,suggestions:l,activeSuggestion:r,onUp:s,onDown:f,focus:d,unfocus:L,go:R,locale:c}}});const $={class:"search-box",role:"search"},_=["placeholder"],V=["onMousedown","onMouseenter"],J=["href"],Q={key:0,class:"parent-page-title"},K={class:"suggestion-row"},Z={class:"page-title"},ee={class:"suggestion-content"};function oe(e,o,t,a,u,l){return h(),p("div",$,[H(b("input",{ref:"input","onUpdate:modelValue":o[0]||(o[0]=r=>e.query=r),"aria-label":"Search",class:C({focused:e.focused}),placeholder:e.locale.placeholder??"Search",autocomplete:"off",spellcheck:"false",onFocus:o[1]||(o[1]=()=>e.focused=!0),onBlur:o[2]||(o[2]=()=>e.focused=!1),onKeyup:[o[3]||(o[3]=v(r=>e.go(e.focusIndex),["enter"])),o[4]||(o[4]=v((...r)=>e.onUp&&e.onUp(...r),["up"])),o[5]||(o[5]=v((...r)=>e.onDown&&e.onDown(...r),["down"]))]},null,42,_),[[j,e.query]]),e.activeSuggestion?(h(),p("ul",{key:0,class:"suggestions",onMouseleave:o[7]||(o[7]=(...r)=>e.unfocus&&e.unfocus(...r))},[(h(!0),p(T,null,S(e.suggestions,(r,i)=>(h(),p("li",{key:i,class:C(["suggestion",{focused:i===e.focusIndex}]),onMousedown:n=>e.go(i),onMouseenter:n=>e.focus(i)},[b("a",{href:r.path,onClick:o[6]||(o[6]=X(()=>{},["prevent"]))},[r.parentPageTitle&&(!e.suggestions[i-1]||e.suggestions[i-1].parentPageTitle!==r.parentPageTitle)?(h(),p("div",Q,y(r.parentPageTitle),1)):z("v-if",!0),b("div",K,[b("div",Z,y(r.title||r.path),1),b("div",ee,[(h(!0),p(T,null,S(r.display,(n,c)=>(h(),p("span",{key:c,class:C(n.type)},y(n.str),3))),128))])])],8,J)],42,V))),128))],32)):z("v-if",!0)])}const ae=B(Y,[["render",oe],["__scopeId","data-v-fd6cd4d5"],["__file","SearchBox.vue"]]);export{ae as default};
